...
export class DraftExpenseItemService {
  updateItem = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const body = schemas.updateDraftExpenseItemSchema.parse(req.body);
      ...
      const item = draft.items.find((entry) => entry.id === req.params.itemId);

      if (!item) {
        return res.status(404).json({
          error: "Draft item not found",
        });
      }
      ...
      const uniqueShares = [
        ...new Map(
          body.shares.map((share) => [share.personId, share]),
        ).values(),
      ];
      ...
      const preparedShares = buildItemShares({
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (preparedShares.length > 0) {
          await tx.expenseDraftItemShare.createMany({
            data: preparedShares.map((share) => ({
              itemId: item.id,
