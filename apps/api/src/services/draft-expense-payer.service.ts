...
export class DraftExpensePayerService {
  updatePayers = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const body = schemas.updateDraftExpensePayersSchema.parse(req.body);
      ...
      const duplicateIds = new Set(body.payers.map((payer) => payer.personId));

      if (duplicateIds.size !== body.payers.length) {
        return res.status(400).json({
          error: "A participant can only be added as a payer once",
        });
      }

      const invalid = body.payers.find(
      ...
      const updatedDraft = await prisma.expenseDraft.findUnique({
        where: {
          id: draft.id,
        },
      ...
      res.json(serializeDraftExpense(updatedDraft!));
    } catch (error) {
      next(error);
    }
  };
