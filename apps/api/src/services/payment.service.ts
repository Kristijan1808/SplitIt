...
import { serializeGroup, serializePaymentFromPayer } from "../utils.js";
...
export class PaymentService {
  listPayments = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const payments = expenses.flatMap((expense) =>
  ...
  createPayment = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const shareAmounts = calculateEqualShares(
        body.amount,
      ...
      const expense = await prisma.expense.create({
        data: {
          ...
          items: {
            create: {
              ordinalNumber: 1,
              name: body.note?.trim() || "Expense",

              price: body.amount,

              shares: {
                create: allPeople.map((participant, index) => ({
      ...
      await prisma.history.create({
        data: {
          ...
          message: `${actor.name} created "${expense.note || "Račun"}" · €${body.amount.toFixed(2)}`,
          newValue: auditValue(actor, expense),
        },
      });
